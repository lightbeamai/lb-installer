...
echo ""
info "Copy the base64 value below into serviceAccountKey to continue onboarding:"
echo ""
cat "$ENCODED_OUTPUT_FILE"
echo ""
echo ""
rm -f "$KEY_OUTPUT_FILE" "$ENCODED_OUTPUT_FILE"
warn "Both local key files have been deleted. The credential above is not stored"
warn "anywhere else — move it into Vault (or wherever this platform's configuration"
warn "is stored) now."
rule
