SELECT
	...
	FALSE AS is_view,
	COALESCE(table_stats.num_rows, 0) AS num_rows,
	...
	END AS is_nullable,
	cc.constraint_type,
	COALESCE(table_stats.table_size, 0) AS table_size
FROM (
	SELECT
		*
	FROM
	...
	LEFT JOIN (
		...
		WHERE
			schemaname NOT in('pg_catalog', 'pg_temp_1', 'pg_toast_temp1', 'information_schema', 'pg_toast')) AS st ON c.table_schema = st.schemaname
	...
	LEFT JOIN (
		SELECT
			pg_stat_all_tables.schemaname, relname AS table_name, n_live_tup AS num_rows, pg_relation_size('"' || schemaname || '"."' || relname || '"') AS table_size
		FROM (
			SELECT
				*
			FROM
			...
			WHERE
				schemaname NOT in('pg_catalog', 'pg_temp_1', 'pg_toast_temp1', 'information_schema', 'pg_toast')) pg_stat_all_tables) table_stats ON c.table_schema = table_stats.schemaname
	AND c.table_name = table_stats.table_name
	LEFT JOIN ( SELECT DISTINCT
			tc.table_schema AS SCHEMA_NAME, kcu.table_name, kcu.column_name, CASE WHEN count(tc.constraint_type) = 1 THEN
